  ...
  >
    <q-card :style="modal.sizes.sm">
      <q-card-section class="row no-wrap justify-between">
        <XTitle label="Visualizar Movimento Financeiro" />
      ...
      <q-card-section>
        <XGroup>
          <XField
            :value="enums.getName('movements.type', item.type)"
            class="col-md-6 col-xs-12"
            label="Tipo"
          />
          <XField
            :value="helpers.floatToMoney(item.value)"
            class="col-md-6 col-xs-12"
            label="Valor"
          />
          <XField
...
const apiParams = computed(() => {
  const params = {
    select: ['id', 'type', 'value', 'category_id', 'description', 'is_active', 'created_by', 'updated_by', 'created_at', 'updated_at'],
    with: ['createdBy:id,name,surname', 'updatedBy:id,name,surname', 'category:id,name'],
  };
